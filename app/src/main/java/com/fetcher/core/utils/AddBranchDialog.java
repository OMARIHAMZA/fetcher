...
public class AddBranchDialog {
    ...
    public AddBranchDialog(Activity mActivity, RefreshActivityCallback activityCallback) {
        ...
        descritpionEditText.setVisibility(View.GONE);
        titleEditText.setHint("Address");
        titleEditText.setFloatingLabelText("Address");
        addProjectButton = mDialog.findViewById(R.id.add_project_button);
        addProjectButton.setOnClickListener(v -> {
            if (titleEditText.getText().toString().equals("")) {
                Toast.makeText(mActivity, "Enter valid Address", Toast.LENGTH_SHORT).show();
                return;
            }
            disableAllView();
            Toast.makeText(mActivity, "Please Wait", Toast.LENGTH_SHORT).show();
            CompanyController.addBranch(mActivity, titleEditText.getText().toString(), new Callback<MessageResponse>() {
                @Override
