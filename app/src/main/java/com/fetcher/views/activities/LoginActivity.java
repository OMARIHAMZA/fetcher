...
import com.fetcher.views.activities.companies.CompanyHomeActivity;
import com.fetcher.views.activities.users.UserHomeActivity;
...
public class LoginActivity extends MasterActivity {
    ...
    protected void onSuccess(@NonNull Call call, @NonNull Response response) {
        if (response.isSuccessful()) {
            ...
            mLoadingDialog.dismiss();
            try {
                Toast.makeText(this, new JSONObject(response.errorBody().string()).getString("error"), Toast.LENGTH_SHORT).show();
            } catch (Exception e1) {
